...
const camera = new THREE.PerspectiveCamera(
  ...
  1, // 카메라 시점 시작
  4000 // 카메라 시점 종료 ➡️ 카메라 시점 시작보다 작거나 시점 종료보다 큰 위치에 존재하면 카메라에 ❌
);
camera.position.set(0, 20, 100); // 도형이 안나온다면 카메라 위치 조절해보기
camera.lookAt(0, 0, 0); // 카메라가 (0, 0, 0) 바라보도록

// 렌더러
const renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true }); // 곡선 매끄럽게
...
controls.enableDamping = true; // 가중치 부여
controls.minDistance = 20; // 스크롤줌 최소값(default=0)
...
const ambientLight = new THREE.AmbientLight(0xffffff, 0.8); // 모든 object 대상으로 전역에서 비춤, 그림자❌
